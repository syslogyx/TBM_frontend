...
abstract class WebSocketServer {
    ...
    protected function split_packet($length, $packet, $user) {
        ...
        //   $this->processAdminSignal($user, $packet);
        // }else
        // {
        //   $this->process($user, $packet);
        // }
                ...
                if (($message = $this->deframe($frame, $user, $headers)) !== FALSE) {
                    if ($user->hasSentClose) {
                        ...
                        if ((preg_match('//u', $message)) || ($headers['opcode'] == 2)) {
                            ...
                            print_r("message 11>>" + $message);
                            $this->process($user, $message);
                        } else {
                            print_r("stderr 22>>" + $message);
        ...
        } else if ($user_origin[1] == "smarttbm2.local") {
            //$this->processAdminSignal($user, $packet);
            if ($user->handlingPartialPacket) {
                $packet = $user->partialBuffer . $packet;
                $user->handlingPartialPacket = false;
                ...
                if (($message = $this->deframe($frame, $user, $headers)) !== FALSE) {
                    if ($user->hasSentClose) {
                        ...
                        if ((preg_match('//u', $message)) || ($headers['opcode'] == 2)) {
                            ...
                            print_r("message 11>>" + $message);
                            // $this->process($user, $message);
                            $this->processAdminSignal($user, $message);
                        } else {
                            print_r("stderr 22>>" + $message);
